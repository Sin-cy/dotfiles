...
search_with_zoxdie() {
    if [ -z "$1" ]; then
        ...
        file="$(fd --type f -I -H -E .git -E .git-crypt -E .cache -E .backup | fzf-tmux -p -w 80% -h 70% --preview='bat --style=plain --color=always {}')"
        if [ -n "$file" ]; then
            nvim "$file"
        fi
    else
        ...
        if [ -n "$lines" ] && [ "$line_count" -eq 1 ]; then
            ...
            file=$(echo "$lines" | fzf-tmux -p --query="$1" -w 80% -h 70% --preview='bat --style=plain --color=always {}')
            if [ -n "$file" ]; then
                nvim "$file"
            fi
        else
